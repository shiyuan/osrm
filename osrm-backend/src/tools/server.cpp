                                             ...
                                             int &max_results_nearest)
{
    ...
    config_options.add_options() //
            ...
            ("port,p",
             ...
             "TCP/IP port") //
            ("threads,t",
             value<int>(&requested_num_threads)->default_value(hardware_threads+1),
...
{
    std::cout << "remove service start" << std::endl;
    // std::system("sh remove_service.sh");
    std::cout << "remove service end" << std::endl;
}
...
int main(int argc, const char *argv[]) try {
    ...
    auto service_handler = std::make_shared<osrm::server::ServiceHandler>(config);
    using osrm::thrift_server::MapServiceServer;
    boost::shared_ptr<MapServiceServer> thrift_server_(new MapServiceServer());
    std::cout << "try start_server..." << std::endl;
    ...
    else
    {
        std::packaged_task<int()> server_task([&] {
            thrift_server_->StartServer(ip_port, service_handler);
            return 0;
        });
        ...
        console_ctrl_function = std::bind(&server::Server::Stop, thrift_server_);
        SetConsoleCtrlHandler(console_ctrl_handler, TRUE);
        util::SimpleLogger().Write() << "running and waiting for requests";
        thrift_server_->StartServer(ip_port, service_handler);
#endif
        util::SimpleLogger().Write() << "initiating shutdown";
        thrift_server_->StopServer();
